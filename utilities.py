...
# PKS1937, you might want to change this. Not that it matters too much, but still...
zem = 0.0
...
def enum_tqdm(iter, total, msg):
    return enumerate(tqdm(iter, ncols=120, total=total, leave=False, desc="[INFO] "+msg))
...
def plot_spec(ax, spec, **kwargs):
    ...
    if not isinstance(xlim, bool) or xlim:
        ...
        wave = spec.x[(spec.x.value > xlim[0].value) &
                      (spec.x.value < xlim[1].value)]
        flux = spec.y[(spec.x.value > xlim[0].value) &
                      (spec.x.value < xlim[1].value)]
        err = spec.dy[(spec.x.value > xlim[0].value) &
                      (spec.x.value < xlim[1].value)]
    else:
        wave = spec.x
        flux = spec.y
...
def convert_to_common_unit(specs, attr):
    ...
    if not specs_in_same_units(ulist):
        ...
        try:
            ...
            raise NameError(
                'Could not convert a spectrum to a common unit, aborting.')
...
def pad_invert(arr, window):
    ...
    n = len(window)//2
    return np.concatenate((2*arr[0] - arr[n:0:-1], arr, 2*arr[-1] - arr[-2:-n-2:-1]))


def compute_corrections(params, n_smooth, y):
    xi_sigma = np.exp(params['a'] * np.log(n_smooth) **
                      2 + params['b'] * np.log(n_smooth) + params['c'])
...
def plot_simulation_results(val, err, xsq, n_rep=5000, nbins=30):
    ...
    n, bins, _ = ax.hist(
        val, facecolor='none', edgecolor='blue', bins=nbins)
    ...
    ax.text(ax.get_xlim()[0] * 1.1 if ax.get_xlim()[0] > 0 else ax.get_xlim()[0] * 0.9, np.max(n),
    ...
    ax.hlines(np.max(n) * 1.25, mu + sigma/2, mu - sigma /
              2, label="Error from gaussian", color='green')
    ax.hlines(np.max(n) * 1.2,  mu + av_error_from_fit/2, mu -
              av_error_from_fit/2, label="Error from fit", color='orange')
    ax.axvline(np.mean(val), np.max(n)/ax.get_ylim()
               [1] * .95, np.max(n)/ax.get_ylim()[1] * 1.05, c='red', ls='-.')
    ax.legend()
...
def gaussian(x, mu, sig):
    return np.exp(-0.5 * np.power((x - mu)/sig, 2.)) / (sig.value * np.sqrt(2 * np.pi))
...
def interpolate_flux(wa: kms, fl: adm, efl: adm, kind=3):
    ...
    #  *should* be equal tho!
    spline_fl = InterpolatedUnivariateSpline(wa.value, fl, k=kind,
                                             ext=0)
    spline_efl = InterpolatedUnivariateSpline(wa.value, efl, k=kind,
                                              ext=0)
...
def split_chunk_independently(spec, n_chunk=None, n_pix_chunk=None, n_pix_smooth=2):
    ...
    assert n_chunk is not None or n_pix_chunk is not None, "No valid input, pass n_chunk or n_pix_chunk"
    ...
    array = np.array_split(np.concatenate(
        (x, y, dy, y_smooth, dy_smooth)).reshape(5, len(x)), n_chunk, axis=1)
    for i in array:
        yield i
