...
def concatenate(*specs):
    utils.convert_to_common_unit(specs, "_xunit")
    ...
    for spec in specs[1:]:
        x_arr = np.concatenate((x_arr, spec.x))
        y_arr = np.concatenate((y_arr, spec.y))
        dy_arr = np.concatenate((dy_arr, spec.dy))

    return gspec.mock_spec(wave=x_arr, flux=y_arr, err=dy_arr)
...
def combine(specs, cliphi=None, cliplo=None, verbose=False):
    ...
    if needrebin:
        ...
        bin_size = max([utils.get_binsize(s.convert_x(utils.kms))
                       for s in specs])
        # Choose largest wavelength bin size of old spectra.
        if verbose:
            print('Rebinning spectra')
        s_rebinned = [s.rebin(dv=bin_size, xstart=wstart,
                              xend=wend, in_place=False) for s in specs]
    else:
        s_rebinned = dc(specs)
    ...
    # Co-addition
    for _, i in utils.enum_tqdm(range(len(common_x)), len(common_x), "Co-adding spectra..."):
        wtot = fltot = ertot = 0.
    ...
    return gspec.generic_spectrum(wave=common_x,
                                  flux=common_y*specs[0].y.unit,
                                  err=common_dy*specs[0].dy.unit,
                                  z_em=specs[0].z_em,
                                  name=specs[0].name + "_CoAdd")
...
def read_from_file(path, name=None, norm=False):
    # This assumes that all files have been opened with Astroocok once, saved as .acs and un-tarred
    ...
    if norm:
        ...
        return gspec.generic_spectrum(wave=wave, flux=flux/cont, err=err/cont, name=name)
    else:
        return gspec.generic_spectrum(wave=wave, flux=flux, err=err, name=name)


def gen_spec_pair(p_sp_1, p_sp_2, shift=1.0 * utils.kms, n_smooth_pix_1=2, n_smooth_pix_2=2):
    _p_sp_1 = dc(p_sp_1)
    _p_sp_2 = dc(p_sp_2)
    _p_sp_2['pos'] += shift
    s1, s2 = gspec.mock_spec(**_p_sp_1), gspec.mock_spec(**_p_sp_2)

    s1.generate()
    s2.generate()

    # one spec only needs smoothing;
    s1.smooth(n_smooth_pix_1)
    # the other spec needs smoothing AND splining;
    s2.spline(n_smooth_pix_2)

    return s1, s2


def merge_spec(*specs):
    # concatenate the flux of all spectra
    flux_all = specs[0].y
    eflux_all = specs[0].dy

    for spec in specs[1:]:
        flux_all = np.concatenate((flux_all, spec.y))
        eflux_all = np.concatenate((eflux_all, spec.dy))

    # generate x array in velocity space
    x_arr = np.arange(len(flux_all)) * \
        specs[0].dispersion.value * specs[0].x.unit
    return gspec.mock_spec(wave=x_arr, flux=flux_all, err=eflux_all)


@au.quantity_input(vstart=utils.kms, vend=utils.kms, dv=utils.kms)
def flux_ccf_update(sp_1, sp_2, vstart, vend, dv, weighted=False):
    ...
    for i, xs in enumerate(x_shift):
        ...
        ccf.append(np.nanmean(y2m*y1m) /
                   np.sqrt(np.nanmean(y2m**2) * np.nanmean(y1m**2)))
        chi2i = (y1-y2)**2/dy**2
