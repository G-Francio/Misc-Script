...
def rebin_spline(x1, y1, x2, interp_kind):
    ...
    subbin_areas = np.array([
        spline.integral(subbin_edges[i], subbin_edges[i + 1])
...
def rebin2d(x1, y1, z1, x2, y2, interp_kind=3):
    ...
    # instantiate spline, s=0 gives interpolating spline
    spline = BoundedRectBivariateSpline(xx,
                                        yy,
                                        zz,
                                        s=0.,
                                        kx=interp_kind,
    ...
    for i in range(ms):
        for j in range(ns):
            ...
            if (x_sub2old[i] == -1 or x_sub2old[i] == m or y_sub2old[j] == -1
                    or y_sub2old[j] == n):
                continue
            else:
                z2[x_sub2new[i]][y_sub2new[j]] += (
                    z1[x_sub2old[i], y_sub2old[j]] * subbin_areas[i, j] /
